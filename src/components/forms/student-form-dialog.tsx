...
export function StudentFormDialog({ open, onOpenChange }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Étudiant créé (POST /students/signup)')
      reset()
      onOpenChange(false)
    },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Inscription via l’API réelle. Le mot de passe n’est jamais stocké ni affiché après
            création (haché dans Keycloak).
          </DialogDescription>
        </DialogHeader>
