...
export function SchoolFormDialog({ open, onOpenChange }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('École créée (POST /schools/signup)')
      reset()
      onOpenChange(false)
    },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            POST /schools/signup (sans token admin). Le mot de passe n’est jamais affiché après création.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
          ...
          <div className="space-y-2"><Label>Mot de passe</Label><Input type="password" {...register('password')} /></div>
